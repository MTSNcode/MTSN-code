...
class SGCLayer(Layer):
    def __init__(self, SGC_weight, degree, input_dim, output_dim, n_heads, attn_drop, ffd_drop, act=tf.nn.elu, residual=False,
                 bias=True, sparse_inputs=False, use_motifs=True, **kwargs):
        super(SGCLayer, self).__init__(**kwargs)
        self.attn_drop = attn_drop
        self.ffd_drop = ffd_drop
        self.act = act
        self.bias = bias
        ...
        xavier_init = tf.contrib.layers.xavier_initializer()
        with tf.variable_scope(self.name + '_vars'):
            self.vars['SGC_weight'] = tf.get_variable('SGC_weight',
                                                   dtype=tf.float32,
    ...
    def SGC_layer(self, seq, SGC_weight, degree, in_sz, out_sz, adj_mat, residual=False,
                     layer_str="", sparse_inputs=False, use_motifs=False, reuse_scope=True):
        """
        SGC model
        """
        with tf.variable_scope('struct_attn', reuse=tf.AUTO_REUSE):
            if sparse_inputs:
                ...
                # seq_fts = tf.expand_dims(tf.sparse_tensor_to_dense(seq), axis=0)
                # seq_fts = tf.expand_dims(seq, axis=0)
                pass
            else:
                seq_fts = tf.layers.conv1d(seq, out_sz, 1, use_bias=False,
                                            name='layer_' + str(layer_str) + '_weight_transform', reuse=reuse_scope)
            # fuse motifs to adj_mat
            # if use_motifs:
            #     node_number = adj_mat.dense_shape[0]
            #     motif_number = motifs[0].dense_shape[0]
            #     weighted_sumed_motifs = tf.sparse_concat(0, motifs)
            #     weighted_sumed_motifs = tf.sparse_reshape(weighted_sumed_motifs, [-1,node_number,node_number]) # [M, N, N]
            #     weighted_sumed_motifs = weighted_sumed_motifs.__mul__(tf.transpose(motifs_weight)) # [N, N]
            ...
            #     adj_mat = tf.sparse_add(adj_mat, weighted_sumed_motifs)

            # seq_fts is feature
            # seq_fts, precompute_time = self.sgc_precompute(seq_fts, adj_mat, degree)
            values, precompute_time = self.sgc_precompute(seq, adj_mat, degree)
            # if use_motifs:
            #     values = tf.reduce_sum(values, axis=0)
            # adj_mat is [N,N] (sparse)
            # values = tf.sparse_tensor_dense_matmul(seq_fts, SGC_weight)
            
            # values = seq_fts
            # values = tf.matmul(seq_fts, SGC_weight)
            
            # use new initer
            # values = tf.matmul(seq_fts, self.vars['SGC_weight'])

            # values = tf.reshape(values, [-1, out_sz])

            # values = tf.expand_dims(values, axis=0) # [1, N, F]

            if self.ffd_drop != 0.0:
                values = tf.nn.dropout(values, 1.0 - self.ffd_drop)  # [N, D]
    ...
    def sgc_precompute(self, features, adj, degree):
        ...
        for i in range(degree):
            try:
                features = tf.sparse_tensor_dense_matmul(adj, features)
            except:
                features = tf.matmul(adj, features)
            # features = self.dense2sparse(features)
        precompute_time = time.time()-t
        return features, precompute_time

    def dense2sparse(self, dense_tensor):
        arr_idx = tf.where(tf.not_equal(dense_tensor, 0))
        # pdb.set_trace()
        tensor_sparse = tf.SparseTensor(arr_idx, tf.gather_nd(dense_tensor, arr_idx), [-1,dense_tensor.get_shape()[1]])
