...
parser.add_argument('--min_time', type=int, nargs='?', default=4, help='min_time step')

parser.add_argument('--max_time', type=int, nargs='?', default=9, help='max_time step')
...
parser.add_argument('--base_model', type=str, nargs='?', default='TIME',
                    help='Base model (TIME/IncSAT)')
...
parser.add_argument('--dataset', type=str, nargs='?', default='ml-10m_new',
                    help='dataset name')  # 'ml-10m_new' # 'UCI' # 'yelp' # 'Epinions' # 'alibaba'
...
train_file = "train.py" if args.base_model == "TIME" else "train_incremental.py"
...
if str2bool(args.run_parallel) and args.base_model == 'TIME':
