...
FLAGS.time_steps = 2
FLAGS.epochs = 20
FLAGS.base_model = 'DySAT'
FLAGS.model = 'default'

# FLAGS.dataset = 'as' # Enron_new as
# init_time = 84-1
# file_name = 'data/as/valid_test'

FLAGS.dataset = 'epinions' # Enron_new as
init_time = 0-1
file_name = 'data/epinions/valid_test'

# FLAGS.dataset = 'facebook_wall'
...
train_edges, train_edges_false, val_edges, val_edges_false, test_edges, test_edges_false, \
    nn_train_edges, nn_train_edges_false, nn_val_edges, nn_val_edges_false, nn_test_edges, \
        nn_test_edges_false = \
...
for epoch in range(FLAGS.epochs):
    ...
    if (epoch + 1) % FLAGS.test_freq == 0:
        ...
        # test_auc, test_f1, test_pr_auc, val_auc, val_f1, val_pr_auc = evaluate_classifier(train_edges,
                                                            #   train_edges_false, val_edges, val_edges_false, test_edges, test_edges_false, emb, emb)
        
        # use our eval code
        val_auc, val_f1, val_pr_auc, test_auc, test_f1, test_pr_auc = predict_model(file_name, emb, '1', init_time+FLAGS.time_steps, False)
        
        print("Epoch {}, Val AUC {}, Val F1 {}, Val PR-AUC {}".format(epoch, val_auc, val_f1, val_pr_auc))
        print("Epoch {}, Test AUC {}, Test F1 {}, Test PR-AUC {}".format(epoch, test_auc, test_f1, test_pr_auc))
        logging.info("Val results at epoch {}: Measure ({}) Val AUC {}, Val F1 {}, Val PR-AUC {}".format(epoch, "HAD", val_auc, val_f1, val_pr_auc))
        logging.info("Test results at epoch {}: Measure ({}) Test AUC {}, Test F1 {}, Test PR-AUC {}".format(epoch, "HAD", test_auc, test_f1, test_pr_auc))
        epoch_auc_test = val_auc
        epoch_auc_val = test_auc
...
test_auc, test_f1, test_pr_auc, val_auc, val_f1, val_pr_auc = evaluate_classifier(train_edges, train_edges_false, val_edges, val_edges_false,
                                                      test_edges, test_edges_false, epochs_embeds[best_epoch],
                                                      epochs_embeds[best_epoch])

# val_auc, val_f1, val_pr_auc, test_auc, test_f1, test_pr_auc = predict_model(file_name, epochs_embeds[best_epoch], '1', init_time+FLAGS.time_steps, False)

nn_test_auc, nn_test_f1, nn_test_pr_auc, nn_val_auc, nn_val_f1, nn_val_pr_auc = evaluate_classifier(nn_train_edges, nn_train_edges_false, nn_val_edges, nn_val_edges_false,
                                                      nn_test_edges, nn_test_edges_false, epochs_embeds[best_epoch],
                                                      epochs_embeds[best_epoch])

print("Best epoch val results Val AUC {}, Val F1 {}, Val PR-AUC {}\n".format(val_auc, val_f1, val_pr_auc))
print("Best epoch test results Test AUC {}, Test F1 {}, Test PR-AUC {}\n".format(test_auc, test_f1, test_pr_auc))
print("New node Best epoch test results Test AUC {}, Test F1 {}, Test PR-AUC {}\n".format(nn_test_auc, nn_test_f1, nn_test_pr_auc))


logging.info("Best epoch Val AUC {}, Val F1 {}, Val PR-AUC {}\n".format(val_auc, val_f1, val_pr_auc))
logging.info("Best epoch Test AUC {}, Test F1 {}, Test PR-AUC {}\n".format(test_auc, test_f1, test_pr_auc))
logging.info("New node Best epoch test results Test AUC {}, Test F1 {}, Test PR-AUC {}\n".format(nn_test_auc, nn_test_f1, nn_test_pr_auc))

with open(output_file,'a+') as f:
    f.write("time steps: {}, epoch time: {}\n".format(num_time_steps, epoch_time))
    f.write("Best epoch val results Val AUC {}, Val F1 {}, Val PR-AUC {}\n".format(val_auc, val_f1, val_pr_auc))
    f.write("Best epoch test results Test AUC {}, Test F1 {}, Test PR-AUC {}\n".format(test_auc, test_f1, test_pr_auc))
    f.write("New node Best epoch test results Test AUC {}, Test F1 {}, Test PR-AUC {}\n".format(nn_test_auc, nn_test_f1, nn_test_pr_auc))
...
emb = np.array(emb)
savemat(SAVE_DIR + '/emb_{}.mat'.format(FLAGS.time_steps - 2))