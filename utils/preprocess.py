...
def load_motifs(dataset_str):
    """Load graph motifs snapshots given the name of dataset"""

    # motifs = loadmat("data/{}/{}".format(dataset_str, "motif_matrix.mat"))['motif']
    motifs = loadmat("data/{}/{}".format(dataset_str, "motif_PGD_matrix.mat"))['motif']
    print("Loaded {}*{} motifs ".format(len(motifs), len(motifs[0])))
    return motifs
...
def normalize_motif_gcn(adj):
    """GCN-based normalization of adjacency matrix (scipy sparse format). Output is in tuple format"""
    adj_ = sp.coo_matrix(adj)
    adj_ = adj + sp.eye(adj.shape[0])
    rowsum = np.array(adj_.sum(1))
    temp = np.power(rowsum, -0.5)
    # temp[np.isinf(temp)] = 1
    degree_mat_inv_sqrt = sp.diags(temp.flatten())
    adj_normalized = adj_.dot(degree_mat_inv_sqrt).transpose().dot(degree_mat_inv_sqrt).tocoo()
    return sparse_to_tuple(adj_normalized)
    # return sparse_to_tuple(sp.coo_matrix(adj))
    # return sparse_to_tuple(sp.coo_matrix(sp.eye(adj.shape[0])))
