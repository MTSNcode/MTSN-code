...
def load_graphs(dataset_str):
    ...
    try:
        ...
        graphs = nx.read_gpickle("data/{}/{}".format(dataset_str, "graphs.pkl"))
...
def get_evaluation_data(adjs, num_time_steps, dataset):
    ...
    try:
        train_edges, train_edges_false, val_edges, val_edges_false, test_edges, test_edges_false, nn_train_edges, nn_train_edges_false, nn_val_edges, nn_val_edges_false, nn_test_edges, nn_test_edges_false = \
        ...
        nn_train_edges, nn_train_edges_false, nn_val_edges, nn_val_edges_false, nn_test_edges, nn_test_edges_false = \
        ...
        np.savez(eval_path, data=np.array([train_edges, train_edges_false, val_edges, val_edges_false,
                                           ...
                                           nn_train_edges, nn_train_edges_false, nn_val_edges, nn_val_edges_false, 
                                           nn_test_edges, nn_test_edges_false]))
...
def create_data_splits(adj, next_adj, val_mask_fraction=0.2, test_mask_fraction=0.6):
    ...
    # new nodes
    nn_all_edge_idx = range(nn_edges.shape[0])
    np.random.shuffle(nn_all_edge_idx)
    nn_num_test = int(np.floor(nn_edges.shape[0]* test_mask_fraction))
    nn_num_val = int(np.floor(nn_edges.shape[0] * val_mask_fraction))
    nn_val_edge_idx = nn_all_edge_idx[:nn_num_val]
    nn_test_edge_idx = nn_all_edge_idx[nn_num_val:(nn_num_val + nn_num_test)]
    nn_test_edges = nn_edges[nn_test_edge_idx]
    nn_val_edges = nn_edges[nn_val_edge_idx]
    nn_train_edges = np.delete(nn_edges, np.hstack([nn_test_edge_idx, nn_val_edge_idx]), axis=0)
    ...
    # new nodes Create train edges.
    nn_train_edges_false = []
    while len(nn_train_edges_false) < len(nn_train_edges):
        idx_i = np.random.randint(0, next_adj.shape[0])
        idx_j = np.random.randint(0, next_adj.shape[0])
        if idx_i == idx_j:
            continue
        if ismember([idx_i, idx_j], edges_next):
            continue
        if ismember([idx_j, idx_i], edges_next):
            continue
        if nn_train_edges_false:
            if ismember([idx_j, idx_i], np.array(nn_train_edges_false)):
                continue
            if ismember([idx_i, idx_j], np.array(nn_train_edges_false)):
                continue
        nn_train_edges_false.append([idx_i, idx_j])

    # new nodes Create test edges.
    nn_test_edges_false = []
    while len(nn_test_edges_false) < len(nn_test_edges):
        idx_i = np.random.randint(0, next_adj.shape[0])
        idx_j = np.random.randint(0, next_adj.shape[0])
        if idx_i == idx_j:
            continue
        ...
        if ismember([idx_j, idx_i], edges_next):
            continue
        if nn_test_edges_false:
            if ismember([idx_j, idx_i], np.array(nn_test_edges_false)):
                continue
            if ismember([idx_i, idx_j], np.array(nn_test_edges_false)):
                continue
        nn_test_edges_false.append([idx_i, idx_j])

    # new nodes Create val edges.
    nn_val_edges_false = []
    while len(nn_val_edges_false) < len(nn_val_edges):
        idx_i = np.random.randint(0, next_adj.shape[0])
        idx_j = np.random.randint(0, next_adj.shape[0])
        if idx_i == idx_j:
            continue
        ...
        if nn_val_edges_false:
            if ismember([idx_j, idx_i], np.array(nn_val_edges_false)):
                continue
            if ismember([idx_i, idx_j], np.array(nn_val_edges_false)):
                continue
        nn_val_edges_false.append([idx_i, idx_j])
    ...
    return list(train_edges), train_edges_false, list(val_edges), val_edges_false, list(test_edges), test_edges_false, list(nn_train_edges), nn_train_edges_false, list(nn_val_edges), nn_val_edges_false, list(nn_test_edges), nn_test_edges_false
