...
class NodeMinibatchIterator(object):
    ...
    """
    def __init__(self, graphs, features, adjs, motifs, placeholders, num_time_steps, context_pairs=None, batch_size=100):
    ...
    def batch_feed_dict(self, batch_nodes):
        ...
        # feed_dict.update({self.placeholders['motifs'][t-min_t]: self.motifs[t] for t in range(min_t, self.num_time_steps)})
        for t in range(min_t, self.num_time_steps):
            feed_dict.update({self.placeholders['motifs{}'.format(t)][t_-min_t]: self.motifs[t][t_] for t_ in range(len(self.motifs[t]))})
